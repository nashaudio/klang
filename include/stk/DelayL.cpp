...
StkFloat DelayL :: tapOut( float tapDelay )
{
    StkFloat outPointer = inPoint_ - tapDelay;  // read chases write
    
    while ( outPointer < 0 )
        outPointer += inputs_.size(); // modulo maximum length
    
    long outPoint_ = (long) outPointer;   // integer part
    if ( outPoint_ == inputs_.size() ) outPoint_ = 0;
    float alpha_ = outPointer - outPoint_; // fractional part
    float omAlpha_ = 1.0f - alpha_;
    
    // First 1/2 of interpolation
    float nextOutput_ = inputs_[outPoint_] * omAlpha_;
